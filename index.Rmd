...
# Positive skewed distribution (e.g., exponential distribution)
data_skewed <- data.frame(
  value = rexp(1000, rate = 1),
  group = 'Positively Skewed'
)

# Symmetric distribution with small variance (e.g., normal distribution)
data_symmetric <- data.frame(
  value = rnorm(1000, mean = 0, sd = 0.5),
  group = 'Symmetric Small Variance'
)

# Combine datasets
data_combined <- rbind(data_skewed, data_symmetric)

# Plot
plot_skewed <- ggplot(data_skewed, aes(x = value)) +
  geom_histogram(binwidth = 0.2, fill = 'skyblue', color = 'black') +
  labs(title = 'Positively Skewed Distribution') +
  theme_minimal()

plot_symmetric <- ggplot(data_symmetric, aes(x = value)) +
  geom_histogram(binwidth = 0.2, fill = 'lightgreen', color = 'black') +
  labs(title = 'Symmetric Distribution with Small Variance') +
  theme_minimal()

# Arrange plots side by side
grid.arrange(plot_skewed, plot_symmetric, ncol = 2)
